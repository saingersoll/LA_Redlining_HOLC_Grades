...
LA_redlining_summary_data <- st_transform(LA_redlining, crs = st_crs(3857))

st_set_crs(LA, 3857)

st_crs(LA) <- 4326

crs(LA_redlining_summary_data)
```
...
**-20 code does not run**

### Joining Data
```{r }
LA_EJ_redline <- st_join(x = LA_redlining_summary_data, y = LA, join = st_intersects, left = FALSE)
```

## % Low Income
```{r }
lowincpct <- LA_EJ_redline %>% 
  group_by(holc_grade) %>% 
   summarize(count = n()) %>% 
  mutate(lowincpct_avg = mean(LA_EJ_redline$LOWINCPCT)) %>% 
          print(lowincpct)
```
## Percentile for Particulate Matter 2.5.

```{r }
particulate_matter <- LA_EJ_redline %>% 
  group_by(holc_grade) %>% 
   summarize(count = n()) %>% 
  mutate(PM25_avg = mean(LA_EJ_redline$PM25)) %>%
           print(PM25_avg)
```
##  Percentile for Low Life Expectancy. 

```{r }
low_life_expectancy <- LA_EJ_redline %>% 
  group_by(holc_grade) %>% 
   summarize(count = n()) %>% 
  mutate(low_life_expct_avg = mean(LA_EJ_redline$LIFEEXPPCT)) %>%
           print(low_life_expectancy$low_life_expct_avg)
```
## Percentile for Air Toxics Cancer Risk

```{r }
air_toxic_Cancer_risk <- LA_EJ_redline %>% 
  group_by(holc_grade) %>% 
   summarize(count = n()) %>% 
  mutate(cancer_avg = mean(LA_EJ_redline$CANCER)) %>%
           print(air_toxic_Cancer_risk$cancer_avg)
```

Please write 2-3 sentences reflecting on these results.
(5 points)

As expected, the redlined districts C and D exhibited higher ratings of average particulate matter in the air, low life expectancy, and cancerous air toxicities. The block groups B and C were the highest for the low income averages.
...
redline_birds <- st_join(x = LA_redlining, y = birds_22, join = st_intersects, left = FALSE)

holc_birds <- redline_birds %>% 
  group_by(holc_grade) %>% 
  summarize(count = n()) %>% 
...
ggplot(holc_birds, aes(x = holc_grade, y = percentage, fill = holc_grade)) + 
  geom_bar(stat = 'identity') +
  labs(title = "HOLC Block Bird Observations 2022") +
...
The HOLC block bird observations are surprising because the lower graded regions, C and D blocks displayed the highest percentage of bird sightings. Typically, these areas are not associated with biodiversity or a surplus of bird habitats. Additionally, individuals who live in these areas are usually a part of the low-income and working class. Those individuals have less time available to allocate to bird watching, therefore it was an unsuspecting outcome that the lower block grades would have the most bird sightings.
