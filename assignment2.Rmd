...
st_transform(LA, crs = st_crs(4326))
```
...
LA_EJ_redline <- st_join(x = LA_redlining, y = LA, join = st_intersects, left = FALSE)

lowincpct <- LA_EJ_redline %>% 
  group_by(holc_grade) %>% 
  mutate(PM25_avg = mean(LOWINCPCT)) %>% 
           print(lowincpct)


particulate_matter <- LA_EJ_redline %>% 
  group_by(holc_grade) %>% 
  mutate(PM25_avg = mean(PM25)) %>%
           print(PM25_avg)

  
low_life_expectancy <- LA_EJ_redline %>% 
  group_by(holc_grade) %>% 
  mutate(PM25_avg = mean(LIFEEXPPCT)) %>%
           print(low_life_expectancy)
  
  
air_toxic_Cancer_risk <- LA_EJ_redline %>% 
  group_by(holc_grade) %>% 
  mutate(PM25_avg = mean(CANCER)) %>%
           print(air_toxic_Cancer_risk)
