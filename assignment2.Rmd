...
ejscreen_state <- st_read("../data/EJSCREEN_2023_BG_StatePct_with_AS_CNMI_GU_VI.gdb")

ejscreen_national<- st_read("../data/EJSCREEN_2023_BG_with_AS_CNMI_GU_VI.gdb")
...
dataset(s):
- EJSCREEN_2023_BG_with_AS_CNMI_GU_VI.gdb  which is the national-level block group data, then the percentile is based off all block group data from 50 states + territories. **95th percentile of national values of wastewater discharge** `P_PWDIS` Anything with a P_ is the percentile  
...
LA_wastewater_discharge <- tm_shape(LAcounty) +
   tm_polygons('PWDIS', 
              #fill.scale = tm_scale_intervals(values ='PuRd'), 
              fill.legend = tm_legend(title = 'Wastewater Discharge by Census Block Groups')) + 
  tm_graticules() +
  tm_shape(LAcountry_blockgroup_centroids)
